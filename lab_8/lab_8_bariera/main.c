...
void *cokolwiek( void *arg){
  int i, moj_id;
  moj_id = *( (int *) arg ); 
  printf("przed 1 bariera - watek %d\n",moj_id);
  bariera();
  printf("przed 2 bariera - watek %d\n",moj_id);
  bariera();
  printf("przed 3 bariera - watek %d\n",moj_id);
  bariera();
  printf("przed 4 bariera - watek %d\n",moj_id);
  bariera();
  printf("KONIEC PRACY - po ostatniej barierze - watek %d\n",moj_id);  
  pthread_exit( (void *)0);
}
