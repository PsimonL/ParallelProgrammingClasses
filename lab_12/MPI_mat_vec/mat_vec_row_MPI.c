...
int main(int argc, char** argv) {
    ...
    if (size > 1) {
        ...
        MPI_Scatter(x, n_wier, MPI_DOUBLE, x_local, n_wier, MPI_DOUBLE, root, MPI_COMM_WORLD);
        memcpy(&x[rank * n_wier], x_local, sizeof(double) * n_wier);
        // ############################################################################################################
...
//        }
        ...
        MPI_Allgather(MPI_IN_PLACE, n_wier, MPI_DOUBLE, x, n_wier, MPI_DOUBLE,
                      MPI_COMM_WORLD);
        ...
        if (rank == root) {
            t1 = MPI_Wtime() - t1;
            printf("Wersja rownolegla MPI z dekompozycj¹ wierszow¹ blokow¹\n");
...
//        }
        ...
        // ############################################################################################################
        if (rank == root) {
            memcpy(&z[n_wier * rank], z, sizeof(double) * n_wier);
            MPI_Gather(MPI_IN_PLACE, n_wier, MPI_DOUBLE, z, n_wier, MPI_DOUBLE,root, MPI_COMM_WORLD);
        } else {
            MPI_Gather(z, n_wier, MPI_DOUBLE, NULL, n_wier, MPI_DOUBLE,root, MPI_COMM_WORLD);
        }
        // ############################################################################################################

        if (rank == root) {
            for (i=0; i<WYMIAR; i++) {
                if (fabs(y[i] - z[i]) > 1.e-9 * z[i]) printf("ERROR! i=%d, y[i]=%lf, z[i]=%lf\n", i, y[i], z[i]);
            }
        }
        ...
        for (i = 0; i < WYMIAR; i++) {
            ...
            MPI_Scatter(&a[i * WYMIAR], n_col, MPI_DOUBLE, &a_local[i * n_col], n_col, MPI_DOUBLE, root, MPI_COMM_WORLD);
        }


        if (rank == root) { // if (rank == 0)
        ...
        // ############################################################################################################
        double* z_local = malloc(sizeof(double) * WYMIAR);
        MPI_Alltoall(z, n_col, MPI_DOUBLE, z_local, n_col, MPI_DOUBLE,MPI_COMM_WORLD);
        memcpy(z, z_local, sizeof(double) * WYMIAR);
        ...
        MPI_Gather(&z[rank * n_col], n_col, MPI_DOUBLE, z_local, n_col,MPI_DOUBLE, root, MPI_COMM_WORLD);
        memcpy(z, z_local, sizeof(double) * WYMIAR);
        // ############################################################################################################
        ...
        if (rank == root) {
            t1 = MPI_Wtime() - t1;
            printf("Werja rownolegla MPI z dekompozycj¹ kolumnow¹ blokow¹\n");
            printf("\tczas wykonania: %lf, Gflop/s: %lf, GB/s> %lf\n", t1,
                   2.0e-9 * ROZMIAR / t1,
